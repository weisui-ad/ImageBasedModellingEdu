...
template <typename T>
void
find_min_max_value (typename core::Image<T>::ConstPtr image, T* vmin, T* vmax);
...
template <typename T>
void
reduce_alpha (typename core::Image<T>::Ptr img); // TODO Blend color?
...
sobel_edge (typename core::Image<T>::ConstPtr img);
...
template <typename T>
void
reduce_alpha (typename core::Image<T>::Ptr img)
{
...
sobel_edge (typename core::Image<T>::ConstPtr img)
{
    ...
    typename core::Image<T>::Ptr out = core::Image<T>::create
        (width, height, chans);
    T* out_ptr = out->get_data_pointer();
    ...
    int thumb_width, int thumb_height)
{
    ...
    typename core::Image<T>::Ptr thumb = core::image::rescale<T>(image,
        core::image::RESCALE_LINEAR, rescale_width, rescale_height);
    thumb = core::image::crop<T>(thumb, thumb_width, thumb_height,
